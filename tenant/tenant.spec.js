...
describe('Tenant API Test', () => {
  ...
  it('Non-admin user should not be able to post to systemsettings', done => {
    shared.login(request, `${shared.rootContext}/rest/v2/tenant/systemsettings`, nonAdminUser.login, nonAdminUser.passwd, err => {
      if (err) {
        return done(err);
      }
      // check the post to system settings api should fail
      request
        .post(`${shared.rootContext}${apiBase}systemsettings`)
        .send({
          css: CSS
