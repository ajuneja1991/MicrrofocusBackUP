...
describe('Notifications API Test - V1', () => {
  const notificationIds = [];
  const
    apiBaseNotification = '/rest/v1/notification',
  ...
  const getUserId = () => new Promise((resolve, reject) => {
    ...
    authRequest.post(`${constants.IDM_CONTEXT}`)
      .set(
        ...
        request.get(`/rest/v1/session/user`).set('X-Auth-Token', res.body.token.id)
          .end((err, response) => {
            if (err) {
              return reject('Failed to get user id');
            }
    ...
    getUserId().then(userId => {
      request
        ...
        .end((err, res) => {
          ...
          notificationIds.push(res.body.data._id);
