...
describe('Dashboard API Test', () => {
  ...
  it('Should check for "no group widget found for the dashboard message" when exporting CSV', done => {
    const api = `${apiBase}csv/${mockData.testDashboardName}`;
    request
      .get(api)
      .expect(404)
      .end((err, res) => {
        ...
        expect(res.body.additionalInfo[1]).to.equal('The CSV cannot be exported since no tabular data found for the test-dashboard dashboard');
        return done();
      });
  });
