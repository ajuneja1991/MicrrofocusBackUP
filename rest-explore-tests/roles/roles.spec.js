...
const { getIDMTestUrl } = require('../../rest-api-tests/helpers/shared');

describe('Foundation Roles Test', () => {
  it('Login admin', done => {
  ...
  it('Create non-default actions with incorrect LValue for resource key data', done => {
    const data = {
  ...
  const checkIDMRoleExistence = (roleName, size = 0) => new Promise((resolve, reject) => {
    const internalRole = {
      ...mockData.role,
      name: roleName
    };
    request.post(apiRole)
      ...
      .end((err, res) => {
        ...
        expect(res.status).to.equal(200);
        expect(res.body.role).to.not.equal(null);
        expect(res.body.role.name).to.equal(internalRole.name);
        ...
        request.get(apiRole.concat(res.body.role.id))
          .end((err, response) => {
            ...
            expect(response.body).to.not.equal(null);
            expect(response.body.role.type).to.equal('foundation');
            expect(response.body.role.name).to.equal(internalRole.name);
            const idmRequest = supertest.agent(getIDMTestUrl());
            const idmRoleURL = `/idm-service/api/scim/organizations/tenant/roles`;
            idmRequest.get(idmRoleURL)
              .end((err, res2) => {
                if (err) {
                  return reject(err);
                }
                const filteredRoles = res2.body.roles.filter(role => role.name === internalRole.name);
                expect(filteredRoles.length).to.equal(size);
                resolve();
              });
          });
      });
  ...
  it('Create Foundation Role with internal role name', async () => {
    try {
      await checkIDMRoleExistence('__NOM_View_Roles', 0);
      await checkIDMRoleExistence('___NOM_View_Roles', 1);
      await checkIDMRoleExistence('NOM_View_Roles__', 1);
      await checkIDMRoleExistence('NOM_View__Roles__', 1);
    } catch (err) {
      throw new Error(err);
    }
  });
  ...
  it('Delete roles (cleanup)', done => {
    request.get(apiRole)
      .end((err, res) => {
        ...
        expect(res.status).to.equal(200);
        expect(res.body.role_list.role.length).to.equal(10);
